...
class TestH5(unittest.TestCase):
    def setUp(self):
        ...
        dts_data[...] = \
            numpy.linspace(0, 120, 120, endpoint=False).reshape(10, 12)
        dts_data2 = \
        ...
        dts_data = \
            self.h5file.create_dataset("/LCLS/ebeamCharge", (1,), dtype='d')
        dts_data = \
            self.h5file.create_dataset("/LCLS/evt41", (1,), dtype='d')
        dts_data = \
            self.h5file.create_dataset(
                "/processing/cheetah/peakinfo-assembled", (3, 4), dtype='d')
        self.dict_test = \
            {'LCLS': {'ebeamCharge': self.h5file["LCLS"]["ebeamCharge"],
